...
export default function CustomerMessages() {
  ...
  useEffect(() => {
    if (!user) return
    const load = async () => {
      // Get all jobs for this customer that have a worker assigned
      const { data: jobsData } = await supabase
        .from('jobs')
        .select('id, title, status, worker_name, worker_id, worker_photo, updated_at')
      ...
      // Fetch unread messages (from worker, not read)
      ...
      const result: ConvJob[] = jobsData
        .filter(j => lastMsgMap[j.id])  // only jobs with at least one message
        .map(j => ({
          ...(j as unknown as Job),
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <div className="flex-1 px-4 py-4 overflow-y-auto">
        {loading ? (
          <div className="flex items-center justify-center h-40 text-sm text-text-muted">Loading...</div>
        ) : convs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-60 gap-3">
            ...
            <p className="text-xs text-text-muted text-center">
              Once a worker bids on your job, chat will appear here.
            </p>
          </div>
        ) : convs.map(conv => (
          <button
          ...
          >
            ...
            <div className="relative shrink-0">
              <div className="w-12 h-12 rounded-full bg-surface border border-border flex items-center justify-center overflow-hidden">
                {conv.worker_photo
                  ? <img src={conv.worker_photo} className="w-full h-full object-cover" />
                  : <span className="text-lg font-bold text-primary">{conv.worker_name?.[0] || 'W'}</span>}
              </div>
              {conv.unreadCount > 0 && (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between mt-1">
                <p className={`text-xs truncate ${conv.unreadCount > 0 ? 'text-text-primary font-medium' : 'text-text-muted'}`}>
                  {conv.lastMessage!.is_customer ? 'You: ' : ''}
                  {conv.lastMessage!.text}
                </p>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium shrink-0 ml-2 ${statusColor(conv.status)}`}>
